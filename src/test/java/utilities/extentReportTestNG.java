...
import testNG.hooks;
	...
	public class extentReportTestNG implements ITestListener {
		...
		public void onTestSuccess(ITestResult result) {
			...
			try {
				String imgPath = s.ScreenShots(result.getName()+"sucess");
		...
		public void onTestSkipped(ITestResult result) {
			...
			try {
				String imgPath = s.ScreenShots(result.getName()+"Skipped");
				test.addScreenCaptureFromPath(imgPath);			
			} 
			catch (Exception e1) {
				e1.printStackTrace();
		...
		public void onTestFailure(ITestResult result) {
			...
			test.log(Status.INFO, result.getThrowable().getMessage());
			try {
				String imgPath = s.ScreenShots(result.getName()+"Failure");
				test.addScreenCaptureFromPath(imgPath);			
			} 
			catch (Exception e1) {
				e1.printStackTrace();
